...
impl Mcp {
    fn definitions() -> Vec<Tool> {
        ...
        [
            ...
            ("soudan_live_targets", "Find native agent sessions in this workspace. OpenCode lists API-addressable saved sessions on a running server, not proof of the session currently displayed by its TUI. Copilot lists the foreground TUI session when started with --ui-server on loopback. Each is reached through its own session API: Codex through its queue, Claude Code through its registered inbox socket. Sessions running in another directory are not listed, because discovery matches the working directory exactly.", json!({}), vec![]),
