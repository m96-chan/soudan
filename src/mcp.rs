...
impl Mcp {
    fn definitions() -> Vec<Tool> {
        ...
        [
            ("soudan_live_targets", "Find existing interactive agent terminals in this workspace. These are open chats, not new headless sessions. Codex targets are reached through Codex's own session API; the others need a connected Kitty bridge.", json!({}), vec![]),
            ("soudan_live_read", "Read an existing chat's current state. Codex returns its thread status and last reply. Other agents return a terminal snapshot, which is a screen holding prompts and status text, not a structured assistant response.", json!({"target":string}), vec!["target"]),
            ("soudan_live_send", "Submit a message directly into an existing interactive chat. Only use when the user has authorized messaging that session. Requires a unique request_id; retries never resubmit uncertain deliveries. Codex accepts a message even mid-turn and answers it in order; other agents are refused while busy or holding a draft. Read the target after sending to see the reply. Do not create automatic reply loops.", json!({"target":string,"text":string,"request_id":string}), vec!["target","text","request_id"]),
